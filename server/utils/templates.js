...
function tabellaVoci(sfondoHeader, testoHeader, sfondoRiga, sfondoAlt, testoPrimario, testoSecondario, fontFamily) {
  ...
  const htmlTemplate = templates[template] || templates.pulito
  const htmlConContatti = htmlTemplate
    .replace(`${citta}${piva ? '  P.IVA ' + piva : ''}${telefono ? '  ' + telefono : ''}`, contattiAzienda)
    .replace(`${citta}${piva ? '  ' + piva : ''}`, contattiAzienda)
    .replace(`${citta}${piva ? '  P.IVA ' + piva : ''}${telefono ? '<br>' + telefono : ''}`, contattiAzienda)

return `<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=500, initial-scale=0.7, shrink-to-fit=yes"><link href="https://fonts.googleapis.com/css2?family=Dancing+Script:wght@600&display=swap" rel="stylesheet"></head><body>${htmlConContatti}</body></html>`
